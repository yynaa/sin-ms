...
export function onBeat(v: number, millis: number, p: p5.default){
    ...
    if (v % 4 == 2 && v < 16*12+2) {
        ...
        currentScene = availableScenes[(Math.floor(v/4))%availableScenes.length] ?? sceneBass
        currentScene.enter(millis, p);
    } else if (v % 2 == 0 && v >= 16*12+2 && v < 16*16+2) {
        const availableScenes = getAvailableScenes();
        currentScene = availableScenes[(Math.floor(v/2))%availableScenes.length] ?? sceneBass
        currentScene.enter(millis, p);
    } else if (v >= 16*16+2) {
        currentScene = null;
        return;
    ...
    const availableShaders: MasterShader[] = [shaderGP, shaderHorizSub, shaderCross, shaderHsvAb];
    if (v < 4*16 && v % 8 == 2) {
        currentShader = availableShaders[Math.floor(v/8)%availableShaders.length] ?? shaderGP
    } else if (v >= 4*16 && v % 4 == 2) {
        currentShader = availableShaders[Math.floor(v/4)%availableShaders.length] ?? shaderGP
    }
...
const sketch = (p: p5.default) => {
    p.setup = () => {
        ...
        // font
        p.loadFont("/fonts/TerminessNerdFontMono-Regular.ttf", "Terminess", undefined, (f) => {
            p.textFont(f, 20);
            assetsToLoad -= 1;
        })

        // shaders
        p.loadShader("/glsl/default.vert", "/glsl/arpCube.frag", (s: p5.default.Shader) => {
            glslArpCube = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/screenFlip.frag", (s: p5.default.Shader) => {
            glslScreenFlip = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/arpBackground.frag", (s: p5.default.Shader) => {
            glslArpBackground = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/bassBackground.frag", (s: p5.default.Shader) => {
            glslBassBackground = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/bassFlip.frag", (s: p5.default.Shader) => {
            glslBassFlip = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/lead.frag", (s: p5.default.Shader) => {
            glslLead = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/drums.frag", (s: p5.default.Shader) => {
            glslDrums = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/zoom.frag", (s: p5.default.Shader) => {
            glslZoom = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/hatSuperposer.frag", (s: p5.default.Shader) => {
            glslHatSuperposer = s;
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/screenFlipZoom.frag", (s: p5.default.Shader) => {
        ...
        videoArp = p.createVideo("/assets/arp.mp4", () => {assetsToLoad -= 1})
        videoArp.volume(0.0);
        videoArp.onended(() => {
            videosToLetPlay -= 1;
            videoArp.onended(() => {})
        ...
        videoBassA = p.createVideo("/assets/bassA.mp4", () => {assetsToLoad -= 1})
        ...
        videoKick = p.createVideo("/assets/kick.mp4", () => {assetsToLoad -= 1})
        ...
        videoSnare = p.createVideo("/assets/snare.mp4", () => {assetsToLoad -= 1})
        ...
        videoHat = p.createVideo("/assets/hat.mp4", () => {assetsToLoad -= 1})
        videoHat.volume(0.0);
        videoHat.onended(() => {
            videosToLetPlay -= 1;
            videoHat.onended(() => {})
        ...
        videoChord = p.createVideo("/assets/chord.mp4", () => {assetsToLoad -= 1})
        ...
        videoLeadA = p.createVideo("/assets/leadA.mp4", () => {assetsToLoad -= 1})
        ...
        videoLeadB = p.createVideo("/assets/leadB.mp4", () => {assetsToLoad -= 1})
        ...
        videoLeadC = p.createVideo("/assets/leadC.mp4", () => {assetsToLoad -= 1})
        ...
        videoLeadD = p.createVideo("/assets/leadD.mp4", () => {assetsToLoad -= 1})
        ...
        videoLeadE = p.createVideo("/assets/leadE.mp4", () => {assetsToLoad -= 1})
        ...
        // shaders
        p.loadShader("/glsl/default.vert", "/glsl/greenAndPurple.frag", (s: p5.default.Shader) => {
            shaderGP = new ShaderGP(p, s);
            assetsToLoad -= 1;
            currentShader = shaderGP;
        })
        p.loadShader("/glsl/default.vert", "/glsl/horizSub.frag", (s: p5.default.Shader) => {
            shaderHorizSub = new ShaderHorizSub(p, s);
            assetsToLoad -= 1;
        })
        p.loadShader("/glsl/default.vert", "/glsl/cross.frag", (s: p5.default.Shader) => {
            shaderCross = new ShaderCross(p, s);
            assetsToLoad -= 1;
            //currentShader = shaderCross;
        })
        p.loadShader("/glsl/default.vert", "/glsl/hsvAb.frag", (s: p5.default.Shader) => {
    ...
    p.draw = () => {
        if (started === null) {
            const videos = [videoHat, videoKick, videoSnare, videoChord, videoArp, videoBassA, videoLeadA, videoLeadB, videoLeadC, videoLeadD, videoLeadE];
            ...
            p.text("sin(ms)", 10 - p.width/2, 30 - p.height/2);
            p.text((assetsToLoad > 0 || videosToLetPlay > 0) ? "Loading..." : "Click to start...", 10 - p.width/2, 50 - p.height/2);
        ...
        if (currentScene !== null) {
            ...
            if (bpm.beat >= 12*16+2) {
                ...
                glslHatSuperposer.setUniform("scale", 2.8 + 0.7 * Math.sin(adjustedMillis/700))
                glslHatSuperposer.setUniform("offset", [5 * Math.sin(adjustedMillis/1900), adjustedMillis/500 + Math.sin(adjustedMillis/1100)])
            }
        ...
        if (DEBUG) {
            ...
            p.text("SMILE v0.2.0 - debug mode", 10 - p.width/2, 30 - p.height/2)
            p.fill(255, 255, 255, 255)
    ...
    p.mousePressed = () => {
        if (started !== null || assetsToLoad > 0 || videosToLetPlay > 0) { return; }
